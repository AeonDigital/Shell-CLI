...
This document provides a complete, minimal walkthrough for building a command line
entrypoint with the shell_cli framework.
...
Create a main executable script, load the framework entrypoint, and define the workspace
root before invoking the engine.
...
Implement any cross-validation and action logic. The validated data will be available
in a command-specific map named `CMD_<PKG>_<TREE>_INPUT`.

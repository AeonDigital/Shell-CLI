...
# shell_cli_metaflag_property_validate_max_array metaflag 'max_array'.
#
# Arguments:
...
shell_cli_metaflag_property_validate_max_array() {
  ...
  if [ "$_array" = "0" ] &&  [ "$fval" != "" ]; then
    SHELL_CLI_METAFLAG_PROPERTY_VALIDATE_ERR_MESSAGE="cannot define 'max_array' for a 'array=false' flag."
    return 1
  fi

...
# shell_cli_preflight_prepare_input — Parse, normalize, and validate raw command-line
# interface parameter flags.
# 
# Arguments
# - $@: Variable array of raw strings containing user-provided command-line arguments.
# 
# Global outputs
# - SHELL_CLI_INPUT_RAW_FLAG: Indexed array holding the isolated raw flag strings
#   discovered.
# - SHELL_CLI_INPUT_RAW_FLAG_ORDER: Indexed array tracking the exact chronological
#   order of parsed long flags.
# - SHELL_CLI_INPUT_RAW_FLAG_ASSOC: Associative matrix mapping normalized long names
#   to their evaluated final values.
# - SHELL_CLI_TRIGGER_HELP: Intercept flag toggled to '1' if help keywords or flags
#   are identified in the stream.
# - SHELL_CLI_TRIGGER_INTERACTIVE: Intercept flag toggled to '1' if interactive wizards
#   are requested by the user.
# 
# Notes
# - Fast-Track Fallback: Short-circuits and returns 0 immediately if 'SHELL_CLI_TRIGGER_HELP'
#   is pre-flight active.
# - Phase 1 (Stream Interception): Scans parameters to separate positional structures
#   from options, intercepting reserved framework triggers.
# - Phase 2 (Payload Expansion): Loops through discovered tokens to split key-value
#   boundaries ('key=value'), falling back to '1' for booleans.
# - Phase 3 (Normalization & Aliasing): Strips notation prefixes and cross-references
#   short names against known mapping matrix aliases.
# - Phase 4 (Constraint Checking): Rejects unknown parameters, blocks duplicated
#   options, and pipes payloads into 'shell_cli_process_flag_value'.
# - Diagnostic Stream: Echoes descriptive syntax, parameter, or validation error
#   traces directly to standard output upon execution failure.
# 
# Returns
# - 0: Success (CLI arguments successfully parsed, normalized, and mapped into the
#   application state matrix).
# - 1: Failure (syntax violation, unknown parameter, duplication error, or downstream
#   value parsing breakdown).
shell_cli_preflight_prepare_input() {
  ...
  # 
  # 1. Extracts each flag passed via the CLI based on its absolute position and identifies
  #    the invocation of modes such as 'help' or 'interactive'.
  local arg=""
  ...
  # 
  # 3. Validates all command flags in the defined order. By this point, all flag
  #    data has been properly validated and processed. We move it into the 'SHELL_CLI_CMD_INPUT'
  #    associative array so that it is easily accessible to the CLI client.

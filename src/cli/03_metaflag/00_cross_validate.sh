...
# shell_cli_metaflag_property_cross_validate_min_max — Cross-validate 'min' and 'max'
# metaflag boundaries.
# 
# Arguments
...
# 
# Global outputs
# - SHELL_CLI_METAFLAG_PROPERTY_VALIDATE_ERR_MESSAGE: Stores descriptive violation
#   message on failure.
# 
# Notes
# - Skips validation and returns 0 if '__cross_min_max=1' is already set in the flag
#   definition.
# - Evaluates boundaries based on data type: numeric comparison for 'int'/'float',
#   chronological epoch comparison for 'date'/'time'/'datetime', and length comparison
#   for 'string' or other types.
# - Mutates the input associative array by setting '__cross_min_max=1' upon successful
#   validation to cache results.
# 
# Returns
...
# 'min_array' and 'max_array' boundaries.
# 
# Arguments
# - fval: Normalized and type-validated input value.
# - fassoc: Name of the associative array containing the flag definition properties.
...
# 
# Notes
# - Skips validation and returns 0 if '__cross_min_array_max_array=1' is already
#   set in the flag definition.
# - Enforcement only triggers when 'is_array' equals 1 and both boundaries are explicitly
#   defined.
# - Mutates the input associative array by setting '__cross_min_array_max_array=1'
#   upon success to cache results.
# 
# Returns

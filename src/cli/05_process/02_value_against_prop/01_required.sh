...
# shell_cli_process_flag_value_against_prop_required — Evaluate the active payload
# value against mandatory presence rules.
# 
# Arguments
# - ruleVal: Configured boolean indicator ('true'/'false' or '1'/'0') specifying
#   if the flag is mandatory.
# 
# Global outputs
# - SHELL_CLI_PROCESS_FLAG_VALUE: Updated with the runtime normalized/transformed
#   counterpart payload upon success.
# - SHELL_CLI_PROCESS_FLAG_VALUE_ERR_MESSAGE: Appended with a formatted framework
#   violation string upon failure.
# 
# Notes
# - Leverages 'shell_cli_metaflag_check_input_required' internally to detect missing
#   or empty structural arguments.

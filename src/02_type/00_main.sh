...
# function for any type that uses an associative array as its underlying 
# type (e.g., enum)
#
# Arguments:
# - value: raw value.
...
shell_cli_type_normalize_main_assoc_types() {
  ...
  local strNormalizated=$(shell_cli_type_normalize_string "$strReturn")
...
declare -g SHELL_CLI_TYPE_VALIDATE_TMP_SELECTED_KEY=""

# Temporarily stores the selected 'value' value identified by 
# validation 'shell_cli_type_validate_main_assoc_types'.
declare -g SHELL_CLI_TYPE_VALIDATE_TMP_SELECTED_VALUE=""

# shell_cli_type_validate_main_assoc_types general validation function 
# for any type that uses an associative array as its underlying 
# type (e.g., enum)
#
# Arguments:
# - value: non empty normalizated value.
...
shell_cli_type_validate_main_assoc_types() {
  ...
  for k in "${!tmpAssoc[@]}"; do
    ...
    if [ "$value" = "$k" ] || [ "$value" = "$v" ]; then
      SHELL_CLI_TYPE_VALIDATE_TMP_SELECTED_KEY="$k"
      SHELL_CLI_TYPE_VALIDATE_TMP_SELECTED_VALUE="$v"
      return 0
    fi
  done
